...
import { createDeviceAction, removeDeviceAction, revokeDeviceAction } from "./actions"
...
export function DevicesClient({ initialDevices }: { initialDevices: Device[] }) {
    ...
    function handleCreateDevice(e: React.FormEvent) {
        ...
        startTransition(async () => {
            const result = await createDeviceAction({ name: newDeviceName.trim() })
            if ("error" in result) {
                setAddError(result.error)
                return
            }

            const createdDev: Device = {
                id: result.id || `dev_${Date.now()}`,
            ...
            showToast(`Device "${result.deviceName}" created successfully.`)
        })
    }
    ...
    function handleConfirmRevoke() {
        ...
        const target = revokeModalDevice
        startTransition(async () => {
            await revokeDeviceAction(target.id)
            setDevices((prev) =>
                prev.map((d) => (d.id === target.id ? { ...d, status: "revoked" } : d))
            )
            showToast(`Device "${target.name}" session has been revoked.`)
    ...
    function handleConfirmRemove() {
        ...
        const target = removeModalDevice
        startTransition(async () => {
            await removeDeviceAction(target.id)
            setDevices((prev) => prev.filter((d) => d.id !== target.id))
            showToast(`Device "${target.name}" removed successfully.`)
                ...
                <DialogContent className="sm:max-w-md">
                    ...
                    <form onSubmit={handleCreateDevice} className="flex flex-col gap-8">
                        ...
                        <DialogFooter className="">
                            ...
                            <Button type="submit" disabled={isPending || !newDeviceName.trim()}>
