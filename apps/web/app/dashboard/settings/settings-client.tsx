...
export function SettingsClient({ settings }: { settings: OrgSettings }) {
    ...
    // Logo Uploader State
    const [hasLogo, setHasLogo] = useState(settings.has_logo)
    ...
    function handleSaveMetadata(e: React.FormEvent) {
        ...
        startTransition(async () => {
            ...
            if (orgName.trim() !== initialOrgName) {
                const nameResult = await updateOrgMetadataAction(orgName.trim())
                if (nameResult?.error) {
            ...
            if (selectedFile) {
                ...
                const logoResult = await uploadLogoAction(formData)
                if (logoResult?.error) {
                    toast.error(logoResult.error)
                    return
                }
            }
