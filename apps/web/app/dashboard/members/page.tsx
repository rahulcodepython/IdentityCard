...
import { type MemberResponse } from "@/schema/members.types"
import { getMembers, deleteMember } from "@/lib/api/members"
...
export default function MembersPage() {
    ...
    const [assignMember, setAssignMember] = useState<MemberResponse | null>(null)
    const [assignSelectedEvents, setAssignSelectedEvents] = useState<string[]>([])

    const [suspendMember, setSuspendMember] = useState<MemberResponse | null>(null)
    const [deleteMemberState, setDeleteMemberState] = useState<MemberResponse | null>(null)
    ...
    const deleteMutation = useMutation({
        ...
        onSuccess: () => {
            ...
            showToast(`${deleteMemberState?.name} has been deleted`)
    ...
    function handleSendInvite() {
        ...
        queryClient.setQueryData<MemberResponse[]>(queryKeys.members(), (prev = []) => [newMember, ...prev])
    ...
    function openAssignModal(member: MemberResponse) {
    ...
    function handleSaveAssignments() {
        ...
        queryClient.setQueryData<MemberResponse[]>(queryKeys.members(), (prev = []) =>
            prev.map((m) =>
    ...
    function handleConfirmSuspend() {
        ...
        const newStatus = suspendMember.status === "Active" ? "Suspended" : "Active"
        queryClient.setQueryData<MemberResponse[]>(queryKeys.members(), (prev = []) =>
    ...
    function handleConfirmDelete() {
        ...
        deleteMutation.mutate(deleteMemberState.id)
    }
    ...
    return (
        <div className="flex flex-col gap-6">
            ...
            <Dialog open={inviteOpen} onOpenChange={setInviteOpen}>
                <DialogContent className="sm:max-w-lg min-h-135 max-h-[85vh] flex flex-col justify-between overflow-y-auto">
                    <DialogHeader>
            ...
            <Dialog open={!!assignMember} onOpenChange={() => setAssignMember(null)}>
                <DialogContent className="sm:max-w-md min-h-95 max-h-[85vh] flex flex-col justify-between overflow-y-auto">
                    <DialogHeader>
            ...
            <Dialog open={!!deleteMember} onOpenChange={() => setDeleteMemberState(null)}>
                <DialogContent className="sm:max-w-md">
                    <DialogHeader>
                        ...
                        <DialogDescription className="pt-2">
                            ...
                            <span className="text-foreground font-semibold">{deleteMemberState?.name}</span>?
                            This action cannot be undone and will permanently remove their access.
                        </DialogDescription>
                    </DialogHeader>

                    <DialogFooter>
                        <Button variant="outline" onClick={() => setDeleteMemberState(null)}>
                            Cancel
                        </Button>
