// Central query-key registry (template §1.12). Every useQuery/useMutation
// in the app references queryKeys.X(...) — never a hand-written array
// literal — so the key used to write a cache entry and the key used to
// read it are structurally guaranteed to match.

// Filter shapes mirror lib/api/people.ts's PeopleFilter and
// lib/api/attendance.ts's RosterFilter today — query-hooks/*.api.ts (a
// later phase) will own these types once it replaces lib/api/*; defined
// locally here in the meantime so this file has no forward dependency on
// not-yet-created query-hooks files.
