// Package cards generates the ID-card PDF (with an embedded QR code) for
// an event's attendees, on demand — never stored, per the project's
...
// repositories) and is itself a one-directional leaf: nothing in those
// packages imports cards, so events.Handler reaches it through a small
// interface (events.CardSender) rather than events importing this
...
func NewService(
	...
	subEventsService *subevents.Service,
...
// publishing.
func (s *Service) GenerateForPerson(ctx context.Context, orgID, eventID, personID uuid.UUID) ([]byte, error) {
...
func (s *Service) ResendForPerson(ctx context.Context, orgID, eventID, personID uuid.UUID) error {
	event, err := s.events.Get(ctx, orgID, eventID)
	if err != nil {
		return err
	}
	if event.Status != "published" {
		return httpx.ErrConflict("cards can only be sent once the event is published")
	}
...
// events.Handler.Publish, runs it in a goroutine so publishing doesn't
...
func (s *Service) SendForEvent(ctx context.Context, orgID, eventID uuid.UUID) {
	event, err := s.events.Get(ctx, orgID, eventID)
	if err != nil {
	...
	attendees, err := s.people.List(ctx, orgID, eventID, people.ListFilter{})
	if err != nil {
...
func (s *Service) generate(ctx context.Context, orgID, eventID, personID uuid.UUID) ([]byte, people.PersonResponse, error) {
	event, err := s.events.Get(ctx, orgID, eventID)
	if err != nil {
	...
	if err != nil {
		return nil, people.PersonResponse{}, httpx.ErrInternal()
	}

	photo := fetchPersonPhoto(ctx, derefOrEmpty(person.ImageURL))
...
func (s *Service) sendCard(person people.PersonResponse, event events.EventResponse, pdfBytes []byte) error {
