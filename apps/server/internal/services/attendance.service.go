...
func NewAttendanceService(cfg *config.Config, repo *repositories.AttendanceRepository, eventsService *EventsService, peopleService *PeopleService, subEventsService *SubEventsService) *AttendanceService {
	return &AttendanceService{cfg: cfg, repo: repo, events: eventsService, people: peopleService, subevents: subEventsService}
}
...
func (s *AttendanceService) Scan(ctx context.Context, deviceOrgID, deviceID uuid.UUID, qrToken string) (entities.ScanResponse, error) {
	claims, err := qrtoken.Parse(s.cfg.QRSecret, qrToken)
	if err != nil {
		return entities.ScanResponse{}, utils.NewError(http.StatusBadRequest, "invalid_qr", "QR code is invalid or expired")
	}
	if claims.OrganizationID != deviceOrgID {
		return entities.ScanResponse{}, utils.ErrForbidden("this card belongs to a different organization")
	}

	event, err := s.events.Get(ctx, deviceOrgID, claims.EventID)
	if err != nil {
		return entities.ScanResponse{}, utils.ErrNotFound("event")
	}
	if event.Status != "published" {
		return entities.ScanResponse{}, utils.NewError(http.StatusConflict, "not_published", "event is not published yet")
	}

	person, err := s.people.Get(ctx, deviceOrgID, claims.EventID, claims.PersonID)
	if err != nil {
		return entities.ScanResponse{}, utils.ErrNotFound("person")
	}

	windows, err := s.resolveExpectedWindows(ctx, deviceOrgID, claims.EventID, person, event)
	if err != nil {
		return entities.ScanResponse{}, err
	}
	today := time.Now().Format(timeutil.DateLayout)
	window, permitted := windows[today]
	if !permitted {
		return entities.ScanResponse{}, utils.NewError(http.StatusForbidden, "not_permitted_today", "this person is not permitted to enter today")
	}
	...
	case errors.Is(err, pgx.ErrNoRows):
		...
		if _, err := s.repo.CreateEntry(ctx, deviceOrgID, claims.EventID, person.ID, today, now, status, deviceID); err != nil {
			return entities.ScanResponse{}, utils.ErrInternal()
		}
		return entities.ScanResponse{Direction: "entry", Status: &status, Date: today, EventName: event.Name, Person: person}, nil
	...
	case !existing.ExitAt.Valid:
		...
		return entities.ScanResponse{Direction: "exit", Status: &status, Date: today, EventName: event.Name, Person: person}, nil

	default:
		return entities.ScanResponse{Direction: "already_completed", Date: today, EventName: event.Name, Person: person}, nil
	}
}
...
// AnalyticsService reuses this unfiltered to compute its rollups, so
// there's only one place this roster-building logic lives.
func (s *AttendanceService) BuildRoster(ctx context.Context, orgID, eventID uuid.UUID, filter RosterFilter) ([]entities.RosterEntry, error) {
	event, err := s.events.Get(ctx, orgID, eventID)
	if err != nil {
		return nil, err
	}
	roster, err := s.people.List(ctx, orgID, eventID, PeopleListFilter{SubEventID: filter.SubEventID})
	if err != nil {
	...
	entries := make([]entities.RosterEntry, 0)
	for _, person := range roster {
		...
		for _, day := range event.Days {
			...
			entry := entities.RosterEntry{PersonID: person.ID, PersonName: person.Name, PersonEmail: person.Email, Date: day.Date}
...
// never stored" shape as PeopleService.Export.
func (s *AttendanceService) Export(ctx context.Context, orgID, eventID uuid.UUID, filter RosterFilter) ([]byte, error) {
	...
	for _, e := range entries {
		_ = w.Write([]string{
			...
			peopleDerefOrEmpty(e.EntryAt), peopleDerefOrEmpty(e.EntryStatus),
			peopleDerefOrEmpty(e.ExitAt), peopleDerefOrEmpty(e.ExitStatus),
		})
	}
	w.Flush()
...
// inherits the whole event's. Computed once per person rather than once
// per (person, date) — this is called from a loop over an event's days in
// BuildRoster, and each sub-event lookup is its own DB read.
func (s *AttendanceService) resolveExpectedWindows(ctx context.Context, orgID, eventID uuid.UUID, person entities.PersonResponse, event entities.EventResponse) (map[string]attendanceWindow, error) {
...
func mergeRecord(entry entities.RosterEntry, row dbgen.ListAttendanceForEventRow) entities.RosterEntry {
