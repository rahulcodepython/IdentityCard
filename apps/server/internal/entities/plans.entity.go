...
// replacing each other, see PlansService.Subscribe — plus a computed
// summary of total usable event capacity, mirroring the pooling logic in
// events' service so the frontend doesn't have to re-derive it.
...
// see PlansService.Subscribe — never replaces an existing one, so an org
...
// kind='custom' plan (validated in PlansService.Subscribe once the plan's
// kind is known — a struct tag can't cleanly express that cross-field rule
// for both custom codes, custom_monthly and custom_yearly).
...
// SubscriptionCandidate is the pure-subscriptions-table view of one active
// subscription — deliberately carries no events-table data (plans doesn't
// know about events at all, to avoid an import cycle: events already
// depends on plans). The caller (events' service) does its own
// per-subscription event counting.
