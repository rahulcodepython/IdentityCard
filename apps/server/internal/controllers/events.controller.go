...
func (ctrl *EventsController) Publish(c *fiber.Ctx) error {
    ...
    // goroutine, not ones spawned from inside a handler.
    go func() {
        defer func() {
            if r := recover(); r != nil {
...
// UploadImage stores a PNG/JPEG event image in MinIO — mirrors
// OrganizationsController.UploadLogo.
func (ctrl *EventsController) UploadImage(c *fiber.Ctx) error {
    id, err := parseEventID(c)
    if err != nil {
        return err
    }
    ...
    if err := ctrl.service.UploadImage(c.Context(), middlewares.Claims(c).OrganizationID, id, contentType, fileHeader.Size, file); err != nil {
        return err
    }
    return c.SendStatus(fiber.StatusNoContent)
}

// GetImage streams the event image directly.
func (ctrl *EventsController) GetImage(c *fiber.Ctx) error {
    id, err := parseEventID(c)
    if err != nil {
        return err
    }
    data, contentType, err := ctrl.service.GetImage(c.Context(), middlewares.Claims(c).OrganizationID, id)
    if err != nil {
        return err
    }
...
// UploadOrganizerSignature stores the organizer's signature image.
func (ctrl *EventsController) UploadOrganizerSignature(c *fiber.Ctx) error {
    id, err := parseEventID(c)
    if err != nil {
        return err
    }
    ...
    contentType := fileHeader.Header.Get("Content-Type")
    if err := ctrl.service.UploadOrganizerSignature(c.Context(), middlewares.Claims(c).OrganizationID, id, contentType, fileHeader.Size, file); err != nil {
        return err
    }
    return c.SendStatus(fiber.StatusNoContent)
}

// GetOrganizerSignature streams the organizer signature image.
func (ctrl *EventsController) GetOrganizerSignature(c *fiber.Ctx) error {
    id, err := parseEventID(c)
    if err != nil {
        return err
    }
    data, contentType, err := ctrl.service.GetOrganizerSignature(c.Context(), middlewares.Claims(c).OrganizationID, id)
    if err != nil {
        return err
    }
    c.Set(fiber.HeaderContentType, contentType)
