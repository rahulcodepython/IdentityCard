...
func (a *App) Publish(ctx context.Context, orgID, id uuid.UUID) (EventResponse, error) {
	txErr := postgres.WithTx(ctx, a.pool, func(tx pgx.Tx) error {
		...
		if err := a.MarkUnjoinedPeopleJoinedAt(ctx, tx, id, pubTime); err != nil {
			return err
		}

		const enqueueJobQuery = `
			INSERT INTO job_outbox (queue, job_type, payload)
			VALUES ('cards', 'send_event_cards', jsonb_build_object('org_id', $1::uuid, 'event_id', $2::uuid));
		`
		if _, err := tx.Exec(ctx, enqueueJobQuery, orgID, id); err != nil {
			return err
		}
		return nil
	})
...
func (a *App) ImportDaysCSV(ctx context.Context, orgID, eventID uuid.UUID, file io.Reader) (DayImportSummary, error) {
	...
	for {
		...
		pgDate, err := utils.ParseDate(dateStr)
		if err != nil {
			summary.Skipped++
...
func parseEventDays(inputs []EventDayInput) ([]parsedDay, error) {
	...
	for i, in := range inputs {
		...
		pgDate, err := utils.ParseDate(in.Date)
		if err != nil {
