...
const (
    CreateEventQuery = `
        WITH ins AS (
            INSERT INTO events (organization_id, start_date, end_date)
            VALUES ($1, $2, $3)
            RETURNING *
        )
    ...
    DeductCreditForEventQuery = `
        ...
        SET credit_balance = credit_balance - 1,
            ...
            updated_at = now()
        WHERE organization_id = $1 AND credit_balance >= 1 AND annual_fee_status != 'past_due'
        RETURNING credit_balance;
    `

    RecordCreditConsumptionQuery = `
        INSERT INTO billing_transactions (organization_id, type, credits_delta, amount, currency, event_id, description)
        VALUES ($1, 'credit_consumed', -1, 0, 'INR', $2, 'Event creation credit deduction');
    `
    ...
    ListFlashEventsOlderThanQuery = `
        ...
        FROM (
            ...
            WHERE e.start_date = e.end_date AND d.date < $1::date
        ) e;
    `
