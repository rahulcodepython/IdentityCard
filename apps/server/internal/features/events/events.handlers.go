...
func (h *App) CreateHandler(c *fiber.Ctx) error {
	...
	if err != nil {
		if errors.Is(err, ErrInvalidDateFormat) || errors.Is(err, ErrInvalidDateRange) || errors.Is(err, ErrInvalidStatus) {
			return utils.ErrBadRequest(c, err.Error(), err)
		}
...
func (h *App) UpdateHandler(c *fiber.Ctx) error {
	...
	if err != nil {
		if errors.Is(err, ErrInvalidEventID) || errors.Is(err, ErrInvalidDateFormat) || errors.Is(err, ErrInvalidDateRange) || errors.Is(err, ErrInvalidStatus) {
			return utils.ErrBadRequest(c, err.Error(), err)
		}
		if errors.Is(err, ErrEventNotFound) {
			return utils.ErrNotFound(c, err.Error(), err)
