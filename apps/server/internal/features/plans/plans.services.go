...
func (a *App) Purchase(ctx context.Context, orgID uuid.UUID, req PurchaseRequest) (BillingResponse, error) {
	...
	a.invalidateOrgBillingCache(ctx, orgID)
...
func (a *App) invalidateOrgBillingCache(ctx context.Context, orgID uuid.UUID) {
	if a.cache != nil {
		_ = a.cache.Delete(ctx, fmt.Sprintf("org:billing_access:%s", orgID.String()))
	}
}
...
func toBillingResponse(b BillingDB, plan PlanDB) BillingResponse {
	...
	return BillingResponse{
		...
		PeriodStart:   utils.FormatDateDirect(b.PeriodStart),
		PeriodEnd:     utils.FormatDateDirect(b.PeriodEnd),
		PruneDate:     utils.FormatDateDirect(b.PruneDate),
		Status:        b.Status,
		Amount:        b.Amount,
